...
PLinkQueue createEmptyQueue_link()
{
    ...
    PLinkQueue p = new LINKQUEUE;//创建节点

    p->front = new QueueData;
    //p->front = p->rear = NULL;
    p->rear = p->front;
    p->front->next = NULL;//成员赋值
    return p;
    /*-------end----------*/
...
void enQueue_link(DataType x, PLinkQueue queue)
{
    //将数据元素x插入队尾。实质：生成一个struct node类型的结点，并给相应成员赋值后插入队尾
     //请在此处填写代码，完成相应功能
     /*-------begin----------*/
    QueueData* w = new QueueData;//生成节点
    w->info = x;
    //queue->front = p;

    w->next = NULL;
    queue->rear->next = w;//成员的赋值
    queue->rear = w;//插入队尾
...
DataType deQueue_link(PLinkQueue Q)
{
   ...
   /*-------begin----------*/
    ...
    char l;
    l = k->info;
    Q->front->next = k->next;           //取出Q队列的队首结点，返回该结点的数据元素
    enQueue_link(k, Q);
    return l;
...
void decrypt(char src[], char key[], char dest[])
{
    ...
    */
    PLinkQueue p = createEmptyQueue_link();//初始化一个空队列Q

    int i = 0;
    while (key[i] != '\0')
    {
        enQueue_link(key[i], p);//将key数组的元素逐个插入队列Q
        i++;
    }
    if (isEmptyQueue_link(p))//判断空队
        exit(0);
    i = 0;
    while (src[i] != '\0')// 使用deQueue_link(Q)函数可得到队首秘钥，并使秘钥循环使用
    {
        dest[i] = src[i] + (deQueue_link(p) - '0');//逐个访问src数组的源文，并加密，
        i++;
    }
    dest[i] = '\0';
}
